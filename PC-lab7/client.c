...
void run_client(int sockfd) {
    ...
    while (read(STDIN_FILENO, buf, BUFLEN - 1) > 0 && !isspace(buf[0])) {
        ...
        //printf("CV MAI SUGESTIV");
        do {
            int aux = send(sockfd, buf, byte_count, 0);
            bytes_sent += aux;
            bytes_remaining -= aux;
       ...
       // recv(sockfd, buf, byte_count, 0);
        if (recv(sockfd, buf, byte_count, 0) == 0) {
            break;
        }
...
int main(int argc, char* argv[])
{
    ...
    inet_aton(argv[1], &serv_addr.sin_addr); // "127.0.0.1" => network order , big endinand
